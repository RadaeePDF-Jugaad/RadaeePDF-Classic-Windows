...
namespace RDPDFLib
{
	...
	namespace view
	{
		...
		public interface class IDXDeviceCallback
		{
			virtual void dx_update() = 0;
			virtual bool dx_render() = 0;
		};
		// 控制所有 DirectX 设备资源。
		class DXDevice
		{
		public:
			DXDevice(Windows::UI::Xaml::Controls::SwapChainPanel^ panel, IDXDeviceCallback ^callback);
			...
			void DXDrawBmp(ID2D1Bitmap *bmp, int left, int top, int right, int bottom)
			{
				//m_locker.lock();
				D2D1_RECT_F rect;
				rect.left = left;
				...
				rect.right = right;
				rect.bottom = bottom;
				m_d2dContext->DrawBitmap(bmp, rect);
				//m_locker.unlock();
			}
			void DXDrawRect(int color, int left, int top, int right, int bottom)
			{
				//m_locker.lock();
				D2D1_RECT_F rect;
				rect.left = left;
				...
				rect.bottom = bottom;
				D2D1_COLOR_F clr;
				clr.a = ((color >> 24) & 0xff) / 255.0f;
				clr.r = ((color >> 16) & 0xff) / 255.0f;
				clr.g = ((color >> 8) & 0xff) / 255.0f;
				clr.b = (color & 0xff) / 255.0f;
				ID2D1SolidColorBrush *br = NULL;
				m_d2dContext->CreateSolidColorBrush(clr, &br);
				m_d2dContext->FillRectangle(rect, br);
				br->Release();
				//m_locker.unlock();
			}
			inline Windows::UI::Xaml::Controls::SwapChainPanel ^GetPanel()
			{
		...
		private:
			...
			void dx_resize();
			void dx_calc_size();
			DXGI_MODE_ROTATION dx_get_rotate();
			...
			IDXDeviceCallback ^m_callback;
			Windows::Foundation::IAsyncAction^ m_renderLoopWorker;
			// 渲染循环计时器。
			RDPDFLib::view::DXTimer m_timer;
			DXLocker m_locker;
		};
	}
}